...
fun Routing.participanteRoute() {
    ...
    route("/$ENDPOINT") {
        ...
        authenticate {
            ...
            // Obtencion de lista de participantes que coincidan con lo solicitado  --> GET /api/participantes/WhenData
            get("/WhenData", {
                ...
                response {
                    HttpStatusCode.OK to {
                        description = "Retorna lista de participantes que coincidan con ese valor."
                        body<List<ParticipanteDto>> { }
                    }
                    HttpStatusCode.NotFound to {
                ...
                try {
                    ...
                    if (!column.isNullOrEmpty() && !query.isNullOrEmpty()) {
                        participanteService.getParticipantesBy(column, query).mapBoth(
                            success = { participanteCoincidentes ->
                                call.respond(HttpStatusCode.OK, participanteCoincidentes.toDto())
                            },
                            failure = { error ->
                    ...
                    call.respond(HttpStatusCode.BadRequest, e.message ?: "Excepción de SQL al buscar los participantes requeridas.")
                } catch (e: Exception) {
                    call.respond(HttpStatusCode.InternalServerError, e.message ?: "Error desconocido al buscar los participantes requeridas.")
                }
            }
